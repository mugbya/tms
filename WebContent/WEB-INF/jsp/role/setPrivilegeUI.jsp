...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("[name=subjectIds]").click(function(){
                ...
                <table class="mainForm">
					...
					<thead>
						<tr align="LEFT" valign="MIDDLE" id="TableTitle">
							<td width="300px" style="padding-left: 7px;">
								<!-- 如果把全选元素的id指定为selectAll，并且有函数selectAll()，就会有错。因为有一种用法：可以直接用id引用元素 -->
								<input type="checkbox" id="cbSelectAll" onClick="$('[name=subjectIds]').attr('checked', this.checked)"/>
								<label for="cbSelectAll">全选</label>
							</td>
						</tr>
					</thead>
...
<ul id="tree">
	...
	<li>
		...
		<s:iterator value="subjectList">
			<li>
				<input type="checkbox" name="subjectIds" value="${id}" id="cbc_${id}" <s:property value="%{id in subjectIds ? 'checked' : ''}"/> />
				<label for="cbc_${id}"><span class="folder">${name}</span></label>
			</li>		
		</s:iterator>
		</ul>
	</li>
